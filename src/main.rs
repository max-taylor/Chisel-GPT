...
pub(crate) const VERSION_MESSAGE: &str = concat!(env!("CARGO_PKG_VERSION"),);
    ...
    match &args.sub {
        Some(ChiselParserSub::List) => {
            let sessions = dispatcher
                .dispatch_command(ChiselCommand::ListSessions, &[])
                .await;
            match sessions {
        ...
        Some(ChiselParserSub::Load { id }) | Some(ChiselParserSub::View { id }) => {
            ...
            match dispatcher
                .dispatch_command(ChiselCommand::Load, &[id])
                .await
            {
        ...
        Some(ChiselParserSub::ClearCache) => {
            match dispatcher
                .dispatch_command(ChiselCommand::ClearCache, &[])
                .await
            {
